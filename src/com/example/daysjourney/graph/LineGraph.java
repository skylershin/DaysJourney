...
package com.example.daysjourney.graph;
...
public class LineGraph extends View {
   ...
   public void addPoint(int ind,LinePoint point){
      lines.get(ind).addPoint(point);
      shouldUpdate = true;
      postInvalidate();
   }
   public void shiftPoint(int ind,LinePoint point,int max){
      lines.get(ind).shiftPoint(point,max);
      shouldUpdate = true;
      postInvalidate();
   }
	...
	public void onDraw(Canvas ca) {
		if (fullImage == null || shouldUpdate) {
         ...
         canvas.drawLine(sidePadding,topPadding,getWidth()-sidePadding,topPadding,paint);
         canvas.drawLine(sidePadding,topPadding,sidePadding,getHeight()-bottomPadding,paint);
         canvas.drawLine(getWidth()-sidePadding,topPadding,getWidth()-sidePadding,getHeight()-bottomPadding,paint);
			paint.setAlpha(255);
			...
			for (Line line : lines){
				...
				if (line.isShowingPoints()){
					for (LinePoint p : line.getPoints()){
						...
						canvas.drawCircle(xPixels,yPixels,4,paint);
						paint.setColor(Color.WHITE);
						canvas.drawCircle(xPixels,yPixels,2,paint);
						
						Path path2 = new Path();
						path2.addCircle(xPixels,yPixels,10,Direction.CW);
						p.setPath(path2);
